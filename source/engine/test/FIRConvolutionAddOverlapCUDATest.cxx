...
TEST_F(FIRConvolutionAddOverlapCUDA, halfLowPassFFTDoubleFrequency)
{
    const tfloat64 c_TOLERANCE = 0.00000001;
...
TEST_F(FIRConvolutionAddOverlapCUDA, halfLowPassFFTDoubleFrequencyOctaveUpscale)
{
    const tfloat64 c_TOLERANCE = 0.00000001;
	...
	engine::FIRConvolutionAddOverlapOctaveUpscale *filterCPU[2];
	FIRConvAddOverlapCuda_Data *filterGPU[2];
	for(int fIdx = 0; fIdx < noChannels; fIdx++)
	{
		filterCPU[fIdx] = new engine::FIRConvolutionAddOverlapOctaveUpscale();
		ASSERT_TRUE(filterCPU[fIdx]->init(lpCoeff, c_blockSize + 1, c_blockSize));
		filterGPU[fIdx] = FIRConvAddOverlapCUDA_Init(lpCoeff, c_blockSize + 1, c_blockSize);
		ASSERT_TRUE(filterGPU[fIdx] != NULL);
	}
	...
	do
	{
		...
		if(data.noParts() > 0)
		{
            ...
            ASSERT_TRUE(FIRConvAddOverlapCUDA_OctaveUpscale_Process(inL, outL, filterGPU[0]));
            ASSERT_TRUE(FIRConvAddOverlapCUDA_OctaveUpscale_Process(inR, outR, filterGPU[1]));
