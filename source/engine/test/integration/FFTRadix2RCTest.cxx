...
class FFTRadix2_DIF
{
	public:
		...
		engine::Complex *iDFT(engine::Complex *x);
...
engine::Complex *FFTRadix2_DIF::iDFT(engine::Complex *x)
{
	tfloat64 invN = 1.0 / static_cast<tfloat64>(m_N);
	engine::Complex *X = DFT(x);
	for(int i = 0; i < m_N; i++)
	{
		X[i].R() *= invN;
		X[i].I() *= invN;
...
void FFT8_Radix2_C2R_A_Recursive(tfloat64* xIn, tfloat64* X)
{
	const tfloat64 c_halfSqr = 0.70710678118654752440084436210485;
	tfloat64(*x)[2] = reinterpret_cast<tfloat64(*)[2]>(xIn);
	tfloat64 t[4][2], s[4], aR, aI;
...
void FFT8_Radix2_C2R_A(tfloat64* xIn, tfloat64* X)
{
	tfloat64 Xt[8];
	FFT8_Radix2_C2R_A_Recursive(xIn, Xt);
	X[0] = Xt[0];
	X[4] = Xt[1];
	X[2] = Xt[2];
	X[6] = Xt[3];
...
TEST(FFTRadix2RC, FFTRadix2_C2R_FFT8_A)
{
	const int size = 8;
	const tfloat64 c_TOLERANCE = 0.00000001;
	common::Random* rand = common::Random::instance();
	...
	for(int i = 0; i < size; i++)
	{
		inA[(i << 1) + 0] = rand->randomReal1();
		inA[(i << 1) + 1] = rand->randomReal1();
		inB[i].R() = inA[(i << 1) + 0];
		inB[i].I() = inA[(i << 1) + 1];
	}

	tfloat64* Xa = new tfloat64[16];
	FFT8_Radix2_C2R_A(inA, Xa);
...
TEST(FFTRadix2RC, FFTRadix2_R2C_FFT8_A)
{
	const int size = 8;
	const tfloat64 c_TOLERANCE = 0.00000001;
	common::Random* rand = common::Random::instance();
...
TEST(FFTRadix2RC, FFTRadix2_R2C_FFT8_B)
{
	const int size = 8;
	const tfloat64 c_TOLERANCE = 0.00000001;
	common::Random* rand = common::Random::instance();
...
TEST(FFTRadix2_R2C_C2R, DFT16_Verify)
{
	const int size = 16;
	const tfloat64 c_TOLERANCE = 0.00000001;
	common::Random *rand = common::Random::instance();
	...
	engine::FFTRadix2<tfloat64> fft(size);
	tfloat64 *Xa = fft.DFT(inA);
...
TEST(FFTRadix2_R2C_C2R, iDFT16_Verify)
{
	const int size = 16;
	const tfloat64 c_TOLERANCE = 0.00000001;
	common::Random *rand = common::Random::instance();
	...
	for(int i = 0; i < size; i++)
	{
		inB[i].R() = rand->randomReal1();
		inB[i].I() = 0.0;
	}

	engine::Complex *Xb = DFT_N_Full(inB, size);

	FFTRadix2_DIF fftA(size);
	engine::Complex *outB = fftA.iDFT(Xb);

	for(int i = 0; i < size; i++)
	{
		EXPECT_NEAR(outB[i].R(), Xb[i].R(), c_TOLERANCE);
		EXPECT_NEAR(outB[i].I(), Xb[i].I(), c_TOLERANCE);
	}
...
TEST(FFTRadix2Cuda, blockIndexRecursionA)
{
	...
	for(int idx = 0; idx < maxN >> 1; idx++)
	{
		...
		for(int bitIndex = 4; (1 << bitIndex) <= maxN; bitIndex++)
		{
			int N = 1 << bitIndex;
			int halfN = N >> 1;
			int b = idx / halfN;
...
TEST(FFTRadix2Cuda, blockIndexRecursionB)
{
	const int maxN = 64, noBits = 6;

	for(int idx = 0; idx < maxN >> 1; idx++)
	{
		fprintf(stdout, "%d - ", idx);
		for(int bI = 4; (1 << bI) <= maxN; bI++)
		{
			int bitIndex = noBits - (bI - 4);
			int N = 1 << bitIndex;
			int halfN = N >> 1;
			int b = idx / halfN;
			int c = (idx % halfN) << 1;
