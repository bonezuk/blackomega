...
*/
int32_t ALACDecoderRef::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
{
...
int32_t ALACDecoderRef::Init(omega::engine::whiteomega::Atom::Track *alacAtomTrack)
{
	...
	theConfig.pb = alacAtomTrack->m_alacPb;
	theConfig.mb = alacAtomTrack->m_alacMb;
	theConfig.kb = alacAtomTrack->m_alacKb;
	theConfig.numChannels = alacAtomTrack->m_alacNumChannels;
	theConfig.maxRun = alacAtomTrack->m_alacMaxRun;
	theConfig.maxFrameBytes = alacAtomTrack->m_alacMaxFrameBytes;
	theConfig.avgBitRate = alacAtomTrack->m_alacAvgBitRate;
...
/*
	...
	- the decoded samples are interleaved into the output buffer in the order they arrive in
	  the bitstream
*/
int32_t ALACDecoderRef::Decode( BitBuffer * bits, uint8_t * sampleBuffer, uint32_t numSamples, uint32_t numChannels, uint32_t * outNumSamples )
{
	BitBuffer			shiftBits;
...
*/
int32_t ALACDecoderRef::DataStreamElement( BitBuffer * bits )
{
