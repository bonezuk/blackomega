...
FFTRadix2_R2C_OctaveUpscale::FFTRadix2_R2C_OctaveUpscale() : engine::FFTRadix2_R2C()
{}

//-------------------------------------------------------------------------------------------

FFTRadix2_R2C_OctaveUpscale::~FFTRadix2_R2C_OctaveUpscale()
{}

//-------------------------------------------------------------------------------------------

void FFTRadix2_R2C_OctaveUpscale::reverse(const tfloat64 *x, tfloat64 *y)
{
	for(int i = 0; i < m_N >> 1; i++)
	{
		y[i] = x[m_reverseIndex[i << 1]];
	}
}

//-------------------------------------------------------------------------------------------

void FFTRadix2_R2C_OctaveUpscale::DFT(const tfloat64 *x, tfloat64 *X)
{
	reverse(x, m_xIO);
	FFTRecursive(m_xIO, X, 0, m_N >> 1);
	X[m_N+0] = X[0];
	X[m_N+1] = X[1];
}
